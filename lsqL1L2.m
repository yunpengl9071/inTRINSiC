...
function [f,grad] = lsqL1L2(X,TFs,miRNAs,gene,lambda,lambda1)
    Fvec = X(2:end);
    dimTF = size(TFs);
...
%     
    if lambda > 0
        grad_1 = sum(grad_1_mat,1) + 2 / log(2) * lambda * log2(Fvec') ./ Fvec';
    else
